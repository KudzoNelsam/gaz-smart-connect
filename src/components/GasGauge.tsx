    ...
    <div className="relative" style={{ width: size, height: size }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        ...
        <g transform={`translate(${size / 2 - 10} ${size - 30})`}>
          <path
            d="M4 2h12v1a3 3 0 013 3v13a3 3 0 01-3 3H4a3 3 0 01-3-3V6a3 3 0 013-3V2z"
            fill="none"
            stroke="currentColor"
            strokeWidth="1.5"
          />
      ...
      <div className="absolute bottom-2 left-0 right-0 text-center text-sm font-medium">
        {safeLevel <= 10 ? 
          <span className="text-gas-red">Critical</span> : 
          safeLevel <= 25 ? 
            <span className="text-gas-orange">Low</span> : 
            <span className="text-gas-blue">Normal</span>
        }
      </div>
    </div>
  );
